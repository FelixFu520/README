# GPU知识

### 对应关系

- 显卡、驱动和cuda[对应关系](https://docs.nvidia.com/deploy/cuda-compatibility/index.html#overview)

- cuda和tensorflow[对应关系](https://www.tensorflow.org/install/source#common_installation_problems)

- cuda和pytorch[对应关系](https://pytorch.org/get-started/previous-versions/)


### GPUs环境安装

- 🍀[Ubuntu16.04安装](os.md)

- 🍀[Ubuntu配置IP](ip.md)

- 🍀[修改源](source.md)

- 🍀[装GPU驱动](driver.md)

- 🍀[装CUDA](cuda.md) | [CUDA Samples解释](cudasamples.md)

- 🍀[装Docker](docker.md)

- 🍀[装NV Docker](nvdocker.md)

- 🍀[OpenCV编译安装](opencv.md)

- 🍀[Python版OpenCV安装](pythonopencv.md)

  

- 🍀[JupyterLab安装](jupyterlab.md)

- 🍀[Matlab安装](matlab.md)



- 🍀[TensorRT安装](tensorrt.md)

## IDE环境安装

- [clion debug](clion.md)