...
![img](imgs/v2-4342c554f6498b0de26e566b4cb47e3e_720w.png)
...
![在这里插入图片描述](imgs/1212.png)

在前文网络的基础上，用红色做了注释。residual使用残差结构。什么是残差结构？举个例子在第一层残差结构（其输出为208*208*128），其输入为208*208*64，经过32*1*1和64*3*3的卷积后，其生成的特征图与输入叠加起来。其结构如下：

![在这里插入图片描述](/Users/fusimeng/README/notes/models/imgs/1313.png)
...
对于一个anchorbox，它包含坐标信息（x , y , w , h ）以及置信度，而这有5个信息；同时还会包含是否所有类别的信息，使用one-hot编码。比如说有3个类:person、car、dog。检测的结果是人，那么就编码为[1,0,0]。可见所有类别信息都会被编码，COCO有80个类别的话，便是5+80。所以，对于每个维度的输出，其结果为：S×S×3×（5+80）=S×S×255*S*×*S*×3×（5+80）=*S*×*S*×255.
...
![img](imgs/v2-758b1df9132a9f4b4e0c7def735e9a11_720w.png)
