# 模型列表



## 1⃣️ 机器学习

- 🍀[最小二乘法](multi.md) ｜🍀[Kmeans](kmeans.md) ｜🍀[PCA](pca.md)

## 2⃣️ NN历史

- 🍀[什么是深度学习](dl/notes/whatisdl.md) ｜ 🍀[神经网络发展史](rcnn/history.md) ｜ 🍀[CNN](rcnn/cnn.md)

## 3⃣️ 手动实现DL模型

- 🍀[零基础入门深度学习(1) -感知器](dlbyhand/Perceptron.md)
- 🍀[零基础入门深度学习(2) - 线性单元和梯度下降](dlbyhand/linear_unit.md)
- 🍀[零基础入门深度学习(3) - 神经网络和反向传播算法](dlbyhand/bp.md)
- 🍀[零基础入门深度学习(4) - 卷积神经网络](dlbyhand/cnn_.md)
- 🍀[零基础入门深度学习(5) - 循环神经网络](dlbyhand/hrnn.md)
- 🍀[代码](https://github.com/FelixFu520/dl-by-hand)
- 🍀[参考](https://www.zybuluo.com/hanbingtao/note/485480)

## 4⃣️ CNN

- 🍀[感知器模型](prec.md)｜[感知器](dl/notes/network-pre.md) ｜ 🍀[VGG](vgg.md) ｜ 🍀[ResNet](resnet.md)  ｜🍀[DenseNet](densenet.md)  
- 🍀[激活函数](activation.md)|[激活函数](dl/notes/network-activationfunction.md) ｜🍀[上采样](up.md) ｜🍀[卷积神经网络(CNN)模型结构](dl/notes/network-cnn.md) ｜🍀[平均池化和最大池化区别](dl/notes/network-pool.md)

## 5⃣️ RNN

- 🍀[RNN](rnn1.md) | 🍀[LSTM](lstm.md) ｜ 🍀[GRU](gru.md)
- 🍀[RNN](rnn.md) | 🍀[rcnns](rcnn/rcnns.md)

## 6⃣️ 网络调优

* [拟合问题](dl/notes/network-fitting.md)
* [正则化](dl/notes/network-bn.md)/[2](dl/notes/network-dnnnorm.md)
* [权重初始化](dl/notes/network-init.md)
* [超参调优](dl/network-superparam.md)

## 7⃣️ Yolo系列

- 🍀[YOLO系列简介](yolos.md)
- 🍀Yolo_v1  ：[blog](yolov1.md) | [code](https://github.com/FelixFu520/yolov1) | [代码解读](yolov1_code.md)
- 🍀Yolo_v2 ：[blog](yolov2.md) | [code](https://github.com/FelixFu520/yolov2) | [代码解读](yolov2_code.md)
- 🍀Yolo_v3 ：[blog](yolov3.md) | [code](https://github.com/FelixFu520/yolov3) | [代码解读](yolov3_code.md)
- 🍀Yolo_v4 ：[blog](yolov4.md) | [code](https://github.com/FelixFu520/yolov4) | [代码解读](yolov4_code.md)
  - [mosaic 数据增强](mosaic.md)
- 🍀Yolo_v5 ：[blog](yolov5.md) | [code](https://github.com/FelixFu520/yolov5) | [代码解读](yolov5_code.md)

## 8⃣️ UNet系列

- 🍀[UNet](https://github.com/ShawnBIT/UNet-family)

