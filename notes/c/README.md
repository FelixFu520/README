# [C++](https://isocpp.org)

## [试题总](listall.md)

### 1. C++基础知识

- **详细笔记**
  - 🍀[入门视频](https://github.com/FelixFu520/Cpp-0-1-Resource) ｜[结构图](arch.md)
- 🍀[C、C++和C#区别](butong.md)
  - 🍀[C++](cpp.md)｜[Python](python.md)

- **编译**
  - 🍀编译过程：[C编译链接执行过程](clink.md)|[编译器工作过程](complier.md) | [静态链接和动态链接](link.md)
  - 🍀编译器：[gcc和g++](gcc.md) | [gcc、make、cmake](cmake.md) ｜ [GUN手册](https://gcc.gnu.org/onlinedocs/) [2](http://www.shanghai.ws/gnu/gcc_1.htm)
  - 🍀工具（make和cmake）：[机器人C/C++编程：实用Make与CMake教程](make.md)
    - [makefile手册](https://seisman.github.io/how-to-write-makefile/introduction.html)
    - CmakeLists学习 [1](cmake3.md)&[2](cmakeN.md)&[3](cmake2.md)&[4](cmake4.md) | [CMake手册](https://www.zybuluo.com/khan-lau/note/254724) [官方手册](https://cmake.org/cmake/help/latest/guide/tutorial/index.html#a-basic-starting-point-step-1)
    - [CMakeLists.txt格式](cmakelists.md)

### 2. C++库学习

- **函数**
  - 🍀[atof函数](atof.md)

- **C++语法**
  - [unordered_map](map.md)
  - [auto](auto.md)

## 编程

- #### 链表

- - [反转链表](reverselist.md) | [链表操作](sl.md) ｜ [按顺序合并链表](hb.md)

- #### 堆

- - [最小K个数](k.md)

- #### 模拟

- - [设计LRU缓存](lru.md)

- #### 操作

  - 增：
  - 删：
  - 改：
  - 查：
  - 排序
    - 冒泡排序（Bubble Sort）
    - [插入排序](insert.md)（Insertion Sort）
    - [希尔排序](shell.md)（Shell Sort）
    - 选择排序（Selection Sort）
    - 快速排序（Quick Sort）
    - 归并排序（Merge Sort）
    - 堆排序（Heap Sort）
    - 计数排序（Counting Sort）
    - 桶排序（Bucket Sort）
    - 基数排序（Radix Sort）