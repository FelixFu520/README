...
> 李宏毅学习笔记：error来自两件事，一个是bias，另一个是variance，把每一层CNN网络类似为打靶。简单的model，variance小，复杂的模型，variance大。
>
> 每一层CNN layer是一个平行空间，每一层都有一个bias和variance，model简单，本网络不足以cover整个Data，而复杂的则过分cover Train Data。
